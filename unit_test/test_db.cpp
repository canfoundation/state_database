...
BOOST_AUTO_TEST_CASE(store_by_scope)
{
   ...
   try
   {
      ...
      {
         const auto &itr = _eosdb.find_or_create_table(receiver, scope, table, payer1);
         _eosdb.db_update_by_scope(itr, payer2, "20.0000 EOS", strlen("20.0000 EOS"));
         const auto &new_itr = _eosdb.find_or_create_table(receiver, scope, table, payer2);
         BOOST_REQUIRE(new_itr.value == "20.0000 EOS" && new_itr.payer == payer2);
      }
